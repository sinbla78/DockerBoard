...
const resolvers = {
  ...
  Mutation: {
    // 회원가입
    register: async (_: any, { email, username, password }: {
      email: string;
      username: string;
      password: string;
    }) => {
    ...
    login: async (_: any, { email, password }: {
      ...
      try {
        ...
        // 이메일 인증 확인
        if (!user.isVerified) {
          throw new Error('이메일 인증을 완료해주세요. 인증 이메일을 확인하세요.');
        }
    ...
    verifyEmail: async (_: any, { token }: { token: string }) => {
      try {
        ...
        if (!success) {
          throw new Error('유효하지 않거나 만료된 인증 토큰입니다. 다시 회원가입을 진행해주세요.');
        }
    ...
    // 게시글 작성
    createPost: async (_: any, { title, content }: {
      title: string;
      content: string;
    }, context: any) => {
      try {
